...
|---|---|---|
| `api::direction.direction` | Направления | `title`*, `description`, `href`, `image` |
| `api::event.event` | Мероприятия | `title`*, `slug` (uid из title), `date`, `description`, `photo`, `gallery`, `isPaid`, `price`, `paymentUrl`, `type` enum (`музыка`/`мастер-класс`/`лекция`/`стенд-ап`), `spotsTotal`, `spotsTaken` |
| `api::event-registration.event-registration` | Регистрации | `eventTitle`, `name`*, `phone`*, `email`, `comment`, `eventId`, `paymentStatus` enum (`pending`/`not_required`/`paid`) |
...
| `api::product.product` | Товары | `title`*, `price`, `category` enum (`ceramics`/`clothing`/`jewelry`/`textile`), `image`*, `gallery`, `cartUrl`, `description`, `isAvailable` |
| `api::showroom.showroom` | Шоурум | `heroImage` (источник hero-картинки страницы `/showroom`) |
| `api::cafe-menu-page.cafe-menu-page` | Меню кофейни — постеры (Single Type) | `mainPosterImage`/`mainPosterAlt`, `summerPosterImage`/`summerPosterAlt`, `footnote` |
| `api::menu-item.menu-item` | Позиции меню кофейни | `name`*, `volume`, `price`*, `note`, `category` enum, `season` enum, `order`, `isAvailable` |
| `api::monthly-report.monthly-report` | Ежемесячные отчёты | `month` integer 1–12 *, `year`*, `pdf`*, `summary` |
| `api::annual-report.annual-report` | Годовые отчёты | `year`*, `kind` enum (`content`/`finance`/`nko-activity`/`spending`)*, `pdf`*, `note` |
| `api::legal-document.legal-document` | Документы фонда | `title`*, `category` enum (`requisites`/`foundation`/`privacy`)*, `pdf`*, `order` |
...
- `PUBLIC_READ_UIDS` — `direction`, `event`, `product`, `showroom`, `cafe-menu-page`, `menu-item`, `monthly-report`, `annual-report`, `legal-document` → `find` + `findOne`.
- `PUBLIC_CREATE_UIDS` — `event-registration`, `order` → POST с лендинга.

Свежий инстанс получает все нужные доступы автоматически после первого старта.
...
- `GET /api/cafe-menu-page?populate=*`
- `GET /api/menu-items?filters[isAvailable][$eq]=true&sort=order:asc`
...
## Миграции, которые нужно учесть при первом релизе

Эти изменения уже в коде (схемы), но Strapi воспринимает их как удаление старых столбцов и создание новых. На локалке (SQLite) — пересоздаётся; на проде (Postgres) — нужны бэкап и/или ручная миграция данных.

- **`event.Price` → `event.price`** — старый столбец `Price` исчезнет. На свежей БД это безопасно. На прод-БД с реальными ценами — `pg_dump` перед деплоем; после деплоя цены ввести заново или мигрировать SQL: `INSERT INTO events (..., price) SELECT ..., "Price" FROM events_old`.
- **`event-registration.paymentStatus` enum** — было `pending`, `" not_required"` (с пробелом). Стало `pending`, `not_required`, `paid`. Старые заявки со статусом `' not_required'` перестанут проходить валидацию — обновить SQL-апдейтом или удалить.
- **`event.slug` (uid)** — добавится новый столбец, у старых записей `slug` будет пустым. Фронту `/events/:id` нужно либо переключиться на `/events/:slug`, либо принимать оба.
- **`product.isAvailable` (boolean default true)** — у старых записей будет `true`, ничего не сломается.
- **`direction.href`** — поле опциональное; у старых направлений останется пустым, фронт продолжит работать через автоопределение (`isShowroomDirection`, …). Заполнять можно постепенно.
